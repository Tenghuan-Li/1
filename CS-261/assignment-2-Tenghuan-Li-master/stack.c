...
struct stack* stack_create() {
  //maybe i need use function;
  struct stack* new_sta = malloc (sizeof(struct stack));
  assert(new_sta);
  new_sta->list = list_create();
  return new_sta;
}
...
void stack_free(struct stack* stack) {
  assert(stack);
  //Here we know in the list.c have function to frees
  if(stack->list!=NULL){
...
void* stack_top(struct stack* stack) {
  assert(stack);
  assert(stack->list);
  void* value1 = list_top(stack->list);
...
void* stack_pop(struct stack* stack) {
  ...
  void* value1 = list_pop(stack->list);
  return value1;
}
