...
struct dynarray* dynarray_create() {
  struct dynarray* new_dy = malloc(sizeof(struct dynarray));
  int int_capa = 2; 
  new_dy->data = malloc(int_capa * sizeof(void*));
  new_dy->size = 0;
  new_dy->capacity = int_capa;
  return new_dy;
}
...
void dynarray_insert(struct dynarray* da, void* val) {
  ...
  if( da->size < da->capacity ){
  	...
  	//need malloc a double da->capacity dyna data;
  	int new_capa = da->capacity * 2;
  	void** new_data = malloc( new_capa *  sizeof( void* ));
  	//put the small data to the big data
  	for( int i= 0 ; i < da->size ; i++ ){
  		new_data[i] = da->data[i];
	  }
	//add the new val in the big dyna capa;
